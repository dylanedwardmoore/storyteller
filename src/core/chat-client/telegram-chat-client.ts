...
export const telegramClient: ChatClientConstructor = (apiKey) => {
    ...
    return {
        runModule: (moduleData, convoManagerConstructor) => {
            bot.use(session())
            const convoManager: ConvoManager = convoManagerConstructor(moduleData.module, moduleData.moduleConfig.initialState)

...
const handleErrorIfNotNull = (error: Error) => {
    if (error) {
        log.fatal('Error connecting to mongodb, check that your credentials in .env are correct.')
        throw error
    }
}

const safelyParseEventUpdate: (stringifiedUpdate: string) => EventUpdate = stringifiedUpdate => {
...
const safelyParseEvent: (mongoEventDocument: IEvent) => Event = mongoEventDocument => {
...
const getUserEventHistory: (userId: string, client: MongoClient) => TE.TaskEither<Error, Event[]> = (userId, client) => {
    log.debug(`Getting all user events for user id ${userId}`)
    return TE.tryCatch(
        R.always(
            new Promise(resolve => client.connect(async (error: Error) => {
            handleErrorIfNotNull(error)
            const eventsWithUserId = await EventMongoSchema.find({ id: userId })
                .then(events => {
                    const result = events.map(safelyParseEvent)
                    log.silly(`For user id ${userId} mapped events to `, result)
                    return result
                })
            client.close();
...
const mongoURI = (mongoAuth: MongoStorageAuth) => `mongodb+srv://${mongoAuth.username}:${mongoAuth.password}@${mongoAuth.uri}/${mongoAuth.databaseName}?retryWrites=true&w=majority`;
...
export const makeMongoDBStorageManager: StorageManagerConstructor<MongoStorageAuth> = mongoAuth => {
    ...
    const client = new MongoClient(mongoURI(mongoAuth), { useNewUrlParser: true });
    ...
    return {
        getUserHistory: userId => {
            ...
            const errorHandling: (error: Error) => T.Task<Event[]> = (error: Error) => {
                log.fatal(`Error fetching data from mongodb for user ${userId}, defaulting to an empty event history. Specific error: `, error)
                return T.of([])
            }

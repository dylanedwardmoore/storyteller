...
function evaluateExpression<K, T extends Readonly<boolean | string | number>>(
    nominalExpression: Expression<K, T>,
    onError: ErrorHandler<T>
): T {
    const expressionValueOrError = getNominalValue(nominalExpression)
    const evaluteExpression: (
        val: Either<Error, Readonly<T>>
    ) => Readonly<T> = fold(onError, identity)
...
) => evaluateExpression(textExpression, onError)

export const evaluateCondition = (
    conditionExpression: Condition,
    onError: ErrorHandler<boolean>
) => evaluateExpression(conditionExpression, onError)

export const evaluateNumber = (
    numberExpression: Number,
    onError: ErrorHandler<number>
) => evaluateExpression(numberExpression, onError)

export const evaluateFilePath = (
    filepathExpression: Filepath,
    onError: ErrorHandler<string>
) => evaluateExpression(filepathExpression, onError)
